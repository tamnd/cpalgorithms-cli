...
describes what users get. Pick a format with `-o`, or let cpa choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
cpa <command> -o table   # aligned columns for reading
cpa <command> -o jsonl   # one JSON object per line, for piping
cpa <command> -o json    # a single JSON array
cpa <command> -o csv     # spreadsheet friendly
cpa <command> -o tsv     # tab-separated
cpa <command> -o url     # just the URL column
cpa <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
cpa <command> --fields id,title,url
```
...
```bash
cpa <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
cpa <command>            # a table, because this is a terminal
cpa <command> | wc -l    # JSONL, because this is a pipe
```
