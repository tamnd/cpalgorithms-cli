// Package cli builds the cpa command tree on top of the cpalgorithms library.
package cli
...
type App struct {
	cfg    cpalgorithms.Config
	client *cpalgorithms.Client
...
func Root() *cobra.Command {
	app := &App{cfg: cpalgorithms.DefaultConfig()}

	root := &cobra.Command{
		Use:   "cpa",
		Short: "Browse CP-Algorithms competitive programming articles",
		Long: `cpa reads CP-Algorithms (https://cp-algorithms.com) and returns article
listings as table, JSON, JSONL, CSV, TSV, or URLs.

cpa is an independent tool and is not affiliated with CP-Algorithms.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = cpalgorithms.NewClient(a.cfg)
	return nil
}
